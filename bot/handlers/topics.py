...
async def cb_collect(
    ...
    raw_exclude = await repo.get_setting("geo_exclude", GEO_EXCLUDE_DEFAULT)
...
async def cb_add_chat_manual_start(callback: CallbackQuery, state: FSMContext):
    ...
    await state.update_data(manual_topic_id=topic_id)
    await callback.message.answer(
...
async def _refresh_chats(
    callback: CallbackQuery, repo: Repository, topic_id: int, page: int
):
